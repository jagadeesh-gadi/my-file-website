...
Util.isLogCacheEnvironment = function () {
  var locationRef;
  var hostname;
  var port;
  ...
  port = locationRef.port || '';

  if (hostname === 'classic-local.w3schools.com' && port === '3000') {
    return true;
  }

  if (hostname === 'kte9uk46s.w3schools.com') {
    return true;
  }
...
Util.isLogCacheEnabled = function () {
  return this._logCacheConfig.cache === true && this.isLogCacheEnvironment();
};

Util.logConfig = function (config) {
  config = config || {};

  if (typeof config.cache === 'boolean') {
    this._logCacheConfig.cache = config.cache && this.isLogCacheEnvironment();
  }

  if (Object.prototype.hasOwnProperty.call(config, 'contexts')) {
    if (config.contexts == null) {
      this._logCacheConfig.contexts = null;
    } else if (Array.isArray(config.contexts)) {
      this._logCacheConfig.contexts = config.contexts.slice();
    }
  }
...
Util.logClearCache = function () {
  this._logCache = [];
};

Util.logGetCache = function () {
  return this._logCache.join('\n');
};

Util._shouldCacheLog = function (context) {
  if (!this._logCacheConfig.cache) {
    return false;
  }

  var contexts = this._logCacheConfig.contexts;
...
Util._formatLogCacheData = function (data, logRawData) {
  var snapshot;
  ...
  snapshot = Util.getDictSnapshot(data, false);
...
Util._appendLogCache = function (level, context, message, data, logRawData) {
  var line;

  if (!this._shouldCacheLog(context)) {
    return;
  }

  if (typeof message !== 'string') {
    line = Util.getCurrentUtms().toString() + ' ' + context + ' -> [' + level + '] ' + String(message);
  } else {
    line = Util.getCurrentUtms().toString() + ' ' + context + ' -> [' + level + '] ' + message +
      this._formatLogCacheData(data, logRawData);
  }

  this._logCache.push(line);

  if (this._logCache.length > this._logCacheMaxEntries) {
    this._logCache.splice(0, this._logCache.length - this._logCacheMaxEntries);
  }
};
...
Util.logDebug = function (context, message, data, logRawData) {
  ...
  Util._appendLogCache('debug', context, message, data, logRawData);
...
Util.logWarning = function (context, message, data, logRawData) {
  ...
  Util._appendLogCache('warn', context, message, data, logRawData);
};
...
Util.logError = function (context, message, data, logRawData) {
  ...
  if (typeof message !== 'string') { // handy for traceback logs
    ...
    Util._appendLogCache('error', context, String(message), data, logRawData);
  ...
  Util._appendLogCache('error', context, message, data, logRawData);
};
...
Util.objFieldOnSetCallback = function (params) {
  ...
  if (typeof callback !== 'function') {
    Util.logError('Util', 'objFieldOnSetCallback -> callback is not a function', {
      fieldName: fieldName
    });
    return;
  }
...
Util.isValidFullName = function (fullName) {
  var fullNamePattern = /^[^\s]+(?:\s+[^\s]+)+$/;
